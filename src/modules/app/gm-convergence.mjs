...
import { ANARCHY_ID, TEMPLATES_PATH } from "../constants.mjs";
import { RemoteCall } from "../remotecall.mjs";

const CONVERGENCES = "convergences";
const SETTING_KEY_CONVERGENCES = `${ANARCHY_ID}.${CONVERGENCES}`;
...
export class GMConvergence {
  ...
  constructor() {
    game.settings.register(ANARCHY_ID, CONVERGENCES, {
      scope: "world",
      config: false,
      default: [],
      type: Array
  ...
  async onReady() {
    ...
    this.convergences = game.settings.get(ANARCHY_ID, CONVERGENCES)
  ...
  async setActorConvergence(actor, newConvergence) {
    ...
    game.settings.set(ANARCHY_ID, CONVERGENCES, this.convergences);
  }
