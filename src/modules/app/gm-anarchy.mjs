...
export class GMAnarchy {
  ...
  constructor() {
    game.settings.register(ANARCHY_ID, GM_ANARCHY, {
      scope: "world",
      config: false,
      default: 1,
      type: Number
    });
    game.settings.register(ANARCHY_ID, GM_SCENE_ANARCHY, {
      scope: "world",
      config: false,
      default: 0,
      type: Number
    ...
    this.anarchy = game.settings.get(ANARCHY_ID, GM_ANARCHY);
  }
  ...
  async setAnarchy(newAnarchy) {
    ...
    game.settings.set(ANARCHY_ID, GM_ANARCHY, newAnarchy);
    await this._rebuild();
