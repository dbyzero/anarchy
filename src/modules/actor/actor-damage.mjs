...
import { ANARCHY_ID, TEMPLATE } from "../constants.mjs";
...
const SETTING_KEY_DAMAGE_MODE = `${ANARCHY_ID}.${DAMAGE_MODE}`;
...
export class ActorDamageManager {
  ...
  static onReady() {
    ActorDamageManager.#registerDamageModeSetting();
    ActorDamageManager.#selectDamageMode();
  }
  ...
  static #registerDamageModeSetting() {
    ...
    game.settings.register(ANARCHY_ID, DAMAGE_MODE, {
      scope: "world",
  ...
  static #selectDamageMode() {
    let damageModeCode = game.settings.get(ANARCHY_ID, DAMAGE_MODE)
  ...
  static async sufferDamageArmorAsResistance_Cyberpunk(actor, monitor, damage, success, avoidArmor, sourceActor) {
    ...
    if (Checkbars.useArmor(monitor) && total > 0) {
      ...
      const armorResistance = Math.max(0, ActorDamageManager.#computeArmorResistance(actor) - ignoredArmor)
  ...
  static async sufferDamageArmorAsResistance_Earthdawn(actor, monitor, damage, success, avoidArmor, sourceActor) {
    ...
    if (Checkbars.useArmor(monitor) && !avoidArmor && total > 0) {
      const armorResistance = ActorDamageManager.#computeArmorResistance(actor);
      if (armorResistance > 0) {
        await Checkbars.addCounter(actor, 'armor', 1);
        total -= armorResistance;
      }
    }
    total -= ActorDamageManager.#computeStrengthResistance(actor, monitor);
    total -= Checkbars.resistance(actor, monitor);
    if (total > 0) {
      await Checkbars.addCounter(actor, monitor, total);
    }
  ...
  static #computeArmorResistance(actor) {
  ...
  static #computeStrengthResistance(actor, monitor) {
    switch (monitor) {
