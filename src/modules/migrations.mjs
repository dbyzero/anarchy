import { ANARCHY_SYSTEM, LOG_HEAD, ANARCHY_ID, TEMPLATE } from "./constants.mjs";
...
class _12_3_0_MigrateHack {
  get version() { return '12.3.0' }
  get code() { return 'migrate-hack-config' }
  async migrate() {
    game.settings.register(ANARCHY_ID, 'anarchy-hack', {
      scope: "world",
    ...
    const hackId = game.settings.get(ANARCHY_ID, 'anarchy-hack');
    game.settings.set(ANARCHY_SYSTEM, 'selected-hack', hackId)
  }
}
...
export class Migrations {
  ...
  static init() {
    Hooks.once(ANARCHY_HOOKS.DECLARE_MIGRATIONS, addMigrations => addMigrations(Migrations.listSystemMigrations()))
  }

  static listSystemMigrations() {
    return [
      ...
      new _12_3_0_MigrateHack(),
    ]
  }

  constructor() {
    game.settings.register(ANARCHY_ID, SYSTEM_MIGRATION_CURRENT_VERSION, {
  ...
  migrate() {
    const currentVersion = game.settings.get(ANARCHY_ID, SYSTEM_MIGRATION_CURRENT_VERSION);
    if (foundry.utils.isNewerVersion(game.system.version, currentVersion)) {
      ...
      game.settings.set(ANARCHY_ID, SYSTEM_MIGRATION_CURRENT_VERSION, game.system.version);
    }
    else {
